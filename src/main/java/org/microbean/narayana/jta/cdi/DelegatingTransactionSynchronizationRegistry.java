...
public abstract class DelegatingTransactionSynchronizationRegistry implements TransactionSynchronizationRegistry {
   ...
   * Map} for use by the caller. If there is no transaction currently
   * active, this method will return {@code null}.</p>
   *
   * <p>The {@link Object} returned will return the same hashCode and
   * compare equal to all other objects returned by calling this
   * method from any component executing in the same transaction
   * context in the same application server.</p>
   *
   * <p>The {@link Object#toString()} method returns a {@link String}
   * that might be usable by a human reader to usefully understand the
   * transaction context. The {@link Object#toString()} result is
   * otherwise not defined. Specifically, there is no forward or
   * backward compatibility guarantee of the results of the returned
   * {@link Object}'s {@link Object#toString()} override.</p>
   * 
   * <p>The object is not necessarily serializable, and has no defined
   * behavior outside the virtual machine whence it was obtained.</p>
   *
  ...
  /**
   * Adds or replaces an object in the {@link Map} of resources being
   ...
   * {@link Object#equals(Object) equals(Object)} methods are suitable
   * for use as keys in a {@link Map}. The key and value are not
   * examined or used by the implementation. The general contract of
   * this method is that of {@link Map#put(Object, Object)} for a
   * {@link Map} that supports non-{@code null} keys and null
   * values. For example, if there is already an value associated with
   * the key, it is replaced by the {@code value} parameter.</p>
   *
   * @param key the key for the {@link Map} entry; must not be {@code null}
   *
  ...
  */
  ...
  /**
   * Gest an object from the {@link Map} of resources being managed
   * for the transaction bound to the current thread at the time this
   * method is called.
   *
   * <p>The key should have been supplied earlier by a call to {@link
   * #putResource(Object, Object)} in the same transaction. If the key
   * cannot be found in the current resource {@link Map}, {@code null}
   * is returned. The general contract of this method is that of
   * {@link Map#get(Object)} for a {@link Map} that supports
   * non-{@code null} keys and null values. For example, the returned
   * value is null if there is no entry for the parameter {@code key}
   * or if the value associated with the key is actually {@code
   * null}.</p>
   *
  ...
  /**
   * Registers a {@link Synchronization} instance with special ordering semantics.
   *
   ...
   * be invoked in the transaction context of the transaction bound to
   * the current thread at the time this method is called. Allowable
   * methods include access to resources, e.g. connectors. No access
   * is allowed to "user components" (e.g. timer services or bean
   * methods), as these might change the state of data being managed
   * by the caller, and might change the state of data that has
   * already been flushed by another caller of {@link
   * #registerInterposedSynchronization(Synchronization)}. The general
   * context is the component context of the caller of {@link
   * #registerInterposedSynchronization(Synchronization)}.</p>
   *
   * <p>The {@link Synchronization#afterCompletion(int)} callback will
   * be invoked in an undefined context. No access is permitted to
   * "user components" as defined above. Resources can be closed but
   * no transactional work can be performed with them.</p>
   *
   ...
   * bound to the current thread at the time this method is
   * called.</p>
   *
   * @return the status of the transaction bound to the current thread
